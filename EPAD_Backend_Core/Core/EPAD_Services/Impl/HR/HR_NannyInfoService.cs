...
{
    public class HR_NannyInfoService : BaseServices<HR_NannyInfo, EPAD_Context>, IHR_NannyInfoService
    {
        public HR_NannyInfoService(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }
        ...
        public async Task<List<HR_NannyInfoResult>> GetAllNannyInfo(string[] pEmployeeATIDs, int pCompanyIndex)
        {
            ...
            var dummy = from u in DbContext.HR_User.Where(x => x.CompanyIndex == pCompanyIndex && (empLookup.Count > 0 ? empLookup.Contains(x.EmployeeATID) : true))
                        join e in DbContext.HR_NannyInfo.Where(x => x.CompanyIndex == pCompanyIndex && (empLookup.Count > 0 ? empLookup.Contains(x.EmployeeATID) : true))
            ...
            {
                var rs = _Mapper.Map<HR_NannyInfoResult>(x.User);
        ...
        {
            var dummy = from u in DbContext.HR_User.Where(x => x.CompanyIndex == pCompanyIndex && x.EmployeeType == 5)
                        join e in DbContext.HR_NannyInfo.Where(x => x.CompanyIndex == pCompanyIndex)
                        on u.EmployeeATID equals e.EmployeeATID
        ...
        public async Task<HR_NannyInfoResult> GetNannyInfo(string pEmployeeATID, int pCompanyIndex)
        {
            var dummy = from u in DbContext.HR_User.Where(x => x.CompanyIndex == pCompanyIndex && x.EmployeeATID == pEmployeeATID)
                        join e in DbContext.HR_NannyInfo.Where(x => x.CompanyIndex == pCompanyIndex && x.EmployeeATID == pEmployeeATID)
                        on u.EmployeeATID equals e.EmployeeATID
