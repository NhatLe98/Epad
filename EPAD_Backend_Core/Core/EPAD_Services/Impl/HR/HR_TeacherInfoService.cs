...
{
    public class HR_TeacherInfoService : BaseServices<HR_TeacherInfo, EPAD_Context>, IHR_TeacherInfoService
    {
        public HR_TeacherInfoService(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }

        public async Task<List<HR_TeacherInfoResult>> GetAllTeacherInfo(string[] pEmployeeATIDs, int pCompanyIndex)
        {
            ...
            var dummy = from u in DbContext.HR_User.Where(x => x.CompanyIndex == pCompanyIndex && (empLookup.Count > 0 ? empLookup.Contains(x.EmployeeATID) : true))
                        join e in DbContext.HR_TeacherInfo.Where(x => x.CompanyIndex == pCompanyIndex && (empLookup.Count > 0 ? empLookup.Contains(x.EmployeeATID) : true))
            ...
            {
                var rs = _Mapper.Map<HR_TeacherInfoResult>(x.User);
        ...
        {
            var dummy = from u in DbContext.HR_User.Where(x => x.CompanyIndex == pCompanyIndex && x.EmployeeType == 7)
                        join e in DbContext.HR_TeacherInfo.Where(x => x.CompanyIndex == pCompanyIndex)
                        on u.EmployeeATID equals e.EmployeeATID
        ...
        public async Task<HR_TeacherInfoResult> GetTeacherInfo(string pEmployeeATID, int pCompanyIndex)
        {
            var dummy = from u in DbContext.HR_User.Where(x => x.CompanyIndex == pCompanyIndex && x.EmployeeATID == pEmployeeATID)
                        join e in DbContext.HR_TeacherInfo.Where(x => x.CompanyIndex == pCompanyIndex && x.EmployeeATID == pEmployeeATID)
                        on u.EmployeeATID equals e.EmployeeATID
