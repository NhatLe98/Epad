...
export default class AutoSynchUserComponent extends Mixins(ComponentBase) {
    ...
    dataProcessedExcel = [];
    dataAddExcel = [];
    listExcelFunction = ['AutoSelectExcel'];
    ...
    MaxHeightTable() {
        this.maxHeight = window.innerHeight - 302;
        this.maxHeighttransfer = window.innerHeight - 268;
        this.maxHeightPrivilege = window.innerHeight - 208;
    }
    ...
    async LoadDepartmentTree() {
        // const userType = this.selectUserType;
    ...
    onChangeUserTypeSelect() {
        ...
        if(this.selectUserType && this.selectUserType.length == 1 && this.selectUserType[0] == 2){
                this.columns = this.columns.filter(x => x.prop != 'CardNumber');
        }
        // switch (this.selectUserType) {
        //     case 1:
        //         if (this.clientName == "MAY") {

        //             this.initColumnEmployee_May();
        //         } else {
    ...
    async Tab1View() {
        ...
        //     case 1:
        //         return await userMasterApi
        //             .GetAllUserMachineInfo(departmentIndexs, this.filter, this.listWorkingStatus)
        //             .then((res) => {
        //                 const data = res.data as any;
        //                 this.page = 1;
        //                 this.dataDB = data;
        ...
        //     default:
        //         return await userMasterApi
        //             .GetAllCustomerMachineInfo(this.filter, this.selectUserType, departmentIndexs)
        //             .then((res) => {
        //                 const data = res.data as any;
        //                 this.page = 1;
        //                 this.dataDB = data;
    ...
    async InsertToMachine() {
        let message = '';
        const self = this;
        if (!this.selectAuthenMode) {
            this.$alert(
                this.$t("SelectAuthenMode").toString(),
                this.$t("Notify").toString(),
                { type: "warning" }
            );
        }
        ...
        if (!this.commandRequest.ListSerial.length) {
            this.$alert(
                this.$t("PleaseSelectMachine").toString(),
                this.$t("Notify").toString(),
                { type: "warning" }
        ...
        // this.showOrHideDialogAuthenMode(false);
        this.showDialogAuthenMode = false;
        try {
            await commandApi.UploadUsers(this.commandRequest).then((res) => {
                if (!isNullOrUndefined(res.status) && res.status === 200) {
                    this.getListIPAddress(this.commandRequest.ListSerial.toString()).then(() => {
