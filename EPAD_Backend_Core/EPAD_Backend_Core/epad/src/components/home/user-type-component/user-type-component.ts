...
export default class UserTypeComponent extends Mixins(ComponentBase) {
    page = 1;
    showDialog = false;
    showMessage = false;
    checked = false;
    ...
    initRule() {
        this.rules = {
            ...
            UserTypeId: [
                {
                    required: true,
                    message: this.$t('PleaseInputUserTypeId'),
                    trigger: 'blur',
                },
            ],
        };
    ...
    async getData({ page, filter, sortParams, pageSize }) {
        this.page = page;
        return await userTypeApi.GetUserTypeAtPage(page, filter, pageSize).then((res) => {
    ...
    async Submit() {
        (this.$refs.ruleForm as any).validate(async (valid) => {
            if (!valid) return;
                ...
                if (this.isEdit == true) {
                    return await userTypeApi.UpdateUserType(this.ruleForm).then((res) => {
                    ...
                    return await userTypeApi
                        .AddUserType(this.ruleForm)
                        .then((res) => {
                            (this.$refs.table as any).getTableData(this.page, null, null);
                            this.showDialog = false;
                            this.reset();
