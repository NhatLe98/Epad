...
import { excusedLateEntryApi, ExcusedLateEntryRequest, HR_ExcusedLateEntry } from '@/$api/excused-late-entry-api';
...
export default class ExcusedLateEntryComponent extends Mixins(ComponentBase) {
	page = 1;
	showDialog = false;
	columns = [];
	rowsObj = [];
	...
	updateFunctionBarCSS() {
        ...
        const component1 = document.querySelector('.excused-late-entry__custom-function-bar');  
        // console.log(component1.childNodes)
        const component2 = document.querySelector('.excused-late-entry__data-table'); 
        let childNodes = Array.from(component1.childNodes);
        const component5 = document.querySelector('.excused-late-entry__data-table-function');
        ...
        // (document.querySelector('.excused-late-entry__data-table-function') as HTMLElement).style.height = "0";
    }
	...
	UploadDataFromExcel() {
        ...
        for (let i = 0; i < this.dataAddExcel[0].length; i++) {
         ...
         if (this.dataAddExcel[0][i].hasOwnProperty('Mã ID (*)')) {
            ...
            if (this.dataAddExcel[0][i].hasOwnProperty('Ngày (*)')) {
                a.LateDate = this.dataAddExcel[0][i]['Ngày (*)'] + '';
            } else {
                a.LateDate = '';
            }
        ...
        excusedLateEntryApi.AddExcusedLateEntryFromExcel(arrData).then((res) => {
            if (!isNullOrUndefined((<HTMLInputElement>document.getElementById('fileUpload')))) {
                (<HTMLInputElement>document.getElementById('fileUpload')).value = '';
            }
            ...
            if (!isNullOrUndefined(res.status) && res.status === 200 && res.data == "") {
                ...
                this.importErrorMessage = this.$t('ImportExcusedLateEntryErrorMessage') + res.data.toString() + " " + this.$t('ExcusedLateEntryInfo');
	...
	async getData({ page, filter, sortParams, pageSize }) {
		this.page = page;
		const requestParam: ExcusedLateEntryRequest = {
		...
		return await excusedLateEntryApi.GetExcusedLateEntryAtPage(requestParam).then((res) => {
			const { data } = res as any;
			data.data.forEach(element => {
				element.LateDateString = moment(element.LateDate).format("DD-MM-YYYY");
			});
			return {
				data: data.data,
				total: data.total,
	...
	async Submit() {
		(this.$refs.ruleForm as any).validate(async (valid) => {
			if (!valid) return;
				...
				this.ruleForm.LateDateString = moment(this.ruleForm.LateDate).format("YYYY-MM-DD");
				if (this.isEdit == true) {
					excusedLateEntryApi.UpdateExcusedLateEntry(this.ruleForm).then((res) => {
						(this.$refs.table as any).getTableData(this.page, null, null);
					...
					excusedLateEntryApi.AddExcusedLateEntry(this.ruleForm).then((res) => {
						// console.log(res);
						...
						}else if((res.data as any).Item1 && (res.data as any).Item1 == "ExcusedLateEntryIsExist"){
							var message = this.$t("StudentAlreadyHaveExcusedLateEntry").toString();
