...
export default class DoorManagementComponent extends Mixins(ComponentBase) {
    EmployeeATID = "";
    EmployeeCode = "";
    FullName = "";
    Gender = "";
    ...
    RunIntegrateLog(){
        if (!this.isActive) {
            this.$alertSaveError(null, null, null, this.$t("PleaseSelectDoor").toString())
        }else{
    ...
    getDoorStatus(){
        attendanceLogApi.GetDoorStatus().then(res => {
           this.doorStatus = res.data;
    ...
    connectToRealTimeServer(link) {
        ...
        const thisVue = this;
        this.connection.on("ReceiveStateLog", listData => {
            for (let i = 0; i < listData.length; i++) {
               
                const table = {
                    optime: listData[i].Optime,
                    devicename: listData[i].DeviceName,
    ...
    getUrl(doorIndex){
        const door = this.doorStatus.find(x => x.DoorIndex.toString() == doorIndex);
        if(door){
            return door.Status ?  '/assets/icons/opendoor.png' : '/assets/icons/closedoor.png'
        }else{
            return '/assets/icons/closedoor.png';
        }
    }
    ...
    openDoor() {
        if (!this.isActive) {
            ...
            commandApi.UnlockDoor({
                ListSerial: [],
                Action: 'Unlock Door',
                ListUser: [],
                IsUsingTimeZone: false,
                TimeZone: [],
                DoorLst : [this.isActive + ''],
                AutoOffSecond: !this.PreviousDays || this.PreviousDays == 0 ? 3 : this.PreviousDays
            }).then((res) => {
                if (!isNullOrUndefined(res.status) && res.status === 200) {
                    this.$notify({
                        ...
                        message: this.$t("SendRequestSuccess").toString(),
            ...
            .then((res: any) => {
                if (res.status == 200) {
                    ...
                    for (let i = 0; i < arrLogs.length; i++) {
                        this.tableData.push({
                            optime: arrLogs[i].Optime,
                            devicename: arrLogs[i].DeviceName,
                            doorname: arrLogs[i].DoorName,
                            status: arrLogs[i].Status,
                        });
