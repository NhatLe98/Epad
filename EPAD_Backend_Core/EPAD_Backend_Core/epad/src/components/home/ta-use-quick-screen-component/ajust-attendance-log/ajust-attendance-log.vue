<template>
  <div class="tab" style="height: calc(100vh - 185px)">
    <div  class="tab-filter">
      <el-row>
        ...
        <el-col :span="1" style="float: right">
          <TableToolbar ref="customerDataTableFunction" :gridColumnConfig.sync="columns" />
        </el-col>
      </el-row>
    </div>
    ...
    <div class="tab-grid" style="height: calc(100% - 35px) !important">
      <VisualizeTable
        v-loading="isLoading"
        :columnDefs="columns.filter((x) => x.display)"
        :rowData="dataSource"
        :rowHeight="40"
      ...
      <AppPagination
        ...
        :getData="loadData"
        :total="total"
        ref="taAjustAttendacenLogPagination"
      />
    </div>
    <div class="tab-modal">
      <el-dialog
        ...
        width="1000px"
        :title="
          isEdit ? $t('EditAjustAttendanceLog') : $t('InsertAjustAttendanceLog')
        "
        ...
        >
            ...
            <el-col :span="11" :offset="1">
              <el-form-item prop="EmployeeATID" :label="$t('Employee')">
                <app-select-new
                  ...
                  style="width: 100%"
                  :placeholder="$t('SelectEmployee')"
                  @getValueSelectedAll="getValueSelectedAll"
                  ref="employeeList"
                >
                </app-select-new>
              </el-form-item>
          ...
          <el-row>
            <el-col :span="11">
              <el-form-item
                :label="$t('Day')"
                @click.native="focus('AttendanceDate')"
                prop="AttendanceDate"
              >
                <el-date-picker
                  format="dd/MM/yyyy"
            ...
            <el-col :span="11" :offset="1">
              <el-form-item
                :label="$t('Hour')"
                @click.native="focus('AttendanceTime')"
                prop="AttendanceTime"
              >
          ...
          <el-row>
            <el-col :span="11">
              <el-form-item
                :label="$t('AttendanceVerifyMode')"
                @click.native="focus('AttendanceVerifyMode')"
                prop="AttendanceVerifyMode"
              >
                <el-select
                  filterable
                  :placeholder="$t('SelectAttendanceVerifyMode')"
                  :disabled="isEdit"
                  v-model="ruleForm.VerifyMode"
                >
                  <el-option
                    v-for="item in attendanceVerifyModeLst"
                    :key="item.value"
                    :label="$t(item.label)"
                    :value="item.value"
                  ></el-option>
            ...
            <el-col :span="11" :offset="1">
              <el-form-item
                :label="$t('InOutModeString')"
                @click.native="focus('InOutModeString')"
                prop="InOutModeString"
              >
                <el-select
                  filterable
                  :placeholder="$t('SelectInOutMode')"
                  v-model="ruleForm.InOutMode"
                  :disabled="isEdit"
                >
                  <el-option
                    v-for="item in allInOutMode"
                    :key="item.value"
                    :label="$t(item.label)"
                    :value="item.value"
                  ></el-option>
        ...
        <el-form label-width="168px" label-position="top">
          ...
          <el-form-item>
            <a
              ...
              href="/Files/AjustAttendanceLogError.xlsx"
              download
              >{{ $t("Download") }}</a
            >
          </el-form-item>
...
<script src="./ajust-attendance-log.ts"></script>
<style lang="scss">
.hozitalClass {
  display: flex;
  flex-direction: row;
...
.ta-adjust-attendance-log__filter-employee
  .el-select__tags:not(:has(span span:nth-child(2))) {
  top: 50% !important;
