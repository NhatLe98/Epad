...
import { taLeaveRegistrationApi, TA_LeaveRegistration, TA_LeaveRegistrationDTO, LeaveRegistrationModel, LeaveDurationType } 
    from '@/$api/ta-leave-registration-api';
...
export default class TARegisterForLeave extends Mixins(TabBase) {
    ...
    columnDefs = [
        ...
        {
            headerName: this.$t('DateRegisterLeave'),
            field: 'LeaveDateString',
            pinned: false,
            width: 200,
            sortable: true,
        ...
        // },
        {
            headerName: this.$t('TypeOfLeaveDate'),
            field: 'LeaveDateTypeName',
            pinned: false,
            width: 150,
            sortable: true,
            display: true
        ...
        {
            headerName: this.$t('DurationType'),
            field: 'LeaveDurationTypeName',
            pinned: false,
            width: 150,
            sortable: true,
            display: true
        ...
        {
            headerName: this.$t('DurationTypeDetail'),
            field: 'HaftLeaveTypeName',
            pinned: false,
            width: 150,
            sortable: true,
            display: true
    ...
    async beforeMount() {
        ...
        await taLeaveRegistrationApi.ExportTemplateLeaveRegistration().then((res: any) => {
            // console.log(res);
        });
    ...
    async loadLeaveData(){
        await taLeaveRegistrationApi.GetLeaveDateType().then((res: any) => {
            if(res && res.status == 200 && res.data){
                // console.log(res.data)
                this.listLeaveDateType = res.data.map(x => {
                    return {
                        Index: x.Index,
                        Name: this.$t(x.Name)
                    }
        ...
        await taLeaveRegistrationApi.GetLeaveDurationType().then((res: any) => {
            if(res && res.status == 200 && res.data){
                // console.log(res.data)
                this.listLeaveDurationType = res.data.map(x => {
                    return {
                        Index: x.Index,
                        Name: this.$t(x.Value)
                    }
        ...
        await taLeaveRegistrationApi.GetHaftLeaveType().then((res: any) => {
            if(res && res.status == 200 && res.data){
                // console.log(res.data)
                this.listHaftLeaveType = res.data.map(x => {
                    return {
                        Index: x.Index,
                        Name: this.$t(x.Value)
                    }
    ...
    initFormRules() {
        this.formRules = {
            ...
            LeaveDate: [
                {
                    required: true,
                    message: this.$t('PleaseSelectLeaveDate'),
                    trigger: 'change',
                },
            ],
            LeaveDateType: [
            ...
            HaftLeaveType: [
                {
                    required: true,
                    message: this.$t('PleaseInputType'),
                    trigger: 'change',
                },
            ],
        };
	...
	handleChangeForm(){
        (this.$refs.registerLeaveModel as any).validate();
	}
    ...
    async loadData() {
        ...
        const filterParam: LeaveRegistrationModel = {
            EmployeeATID: null,
            LeaveDate: null,
            LeaveDateType: null,
        ...
        await taLeaveRegistrationApi.GetLeaveRegistration(filterParam).then((res: any) => {
            // console.log(res)
            if(res.status == 200 && res.data){
                this.dataSource = res.data.data.map((x, idx) => {
                    return {
                        ...
                        LeaveDurationTypeName: this.$t(x.LeaveDurationTypeName),
                        HaftLeaveTypeName: this.$t(x.HaftLeaveTypeName),
                    }
                });
    ...
    async doDelete() {
        ...
        await taLeaveRegistrationApi.DeleteLeaveRegistration(selectedIndex).then(async (res) => {
            // console.log(res)
    ...
    async onViewClick() {
        ...
        this.page = 1;
        (this.$refs.registerLeavePagination as any).page = this.page;
        (this.$refs.registerLeavePagination as any).lPage = this.page;
        await this.loadData();
    }

    async onSubmitClick() {
        (this.$refs.registerLeaveModel as any).validate(async (valid) => {
            ...
            (this.formModel as any).LeaveDateString = moment((this.formModel as any).LeaveDate).format('YYYY-MM-DD');
            ...
            // (this.formModel as any).LeaveDate = new Date((this.formModel as any).LeaveDateString);
            ...
            if((this.formModel as any).Index && (this.formModel as any).Index > 0){
                await taLeaveRegistrationApi.UpdateLeaveRegistration(this.formModel).then((res: any) => {
                    // console.log(res)
                    if(res.status && res.status == 200 && res.data){
                ...
                await taLeaveRegistrationApi.AddLeaveRegistration(this.formModel).then((res: any) => {
                    // console.log(res)
                    if(res.status && res.status == 200 && res.data){
                        if(typeof res.data == 'object'){
                            let message = '';
    ...
    onCancelClick() {
        ...
        (this.$refs.registerLeaveModel as any).resetFields();
        (this.$refs.registerLeaveModel as any).clearValidate();
        this.showDialog = false;
        this.formModel = {};
    ...
    UploadDataFromExcel() {
        ...
        for (let i = 0; i < this.dataAddExcel[0].length; i++) {
            ...
            if (this.dataAddExcel[0][i].hasOwnProperty("Loại ngày nghỉ (*)")) {
                a.LeaveDateTypeName = this.dataAddExcel[0][i]["Loại ngày nghỉ (*)"] + "";
            } else {
                a.LeaveDateTypeName = "";
            }
            if (this.dataAddExcel[0][i].hasOwnProperty("Cả ngày") && this.dataAddExcel[0][i]["Cả ngày"] != "") {
                a.LeaveDurationType = LeaveDurationType.LeaveAllShift;
            } else {
                a.LeaveDurationType = LeaveDurationType.LeaveHaftShift;
            }
            if (this.dataAddExcel[0][i].hasOwnProperty("Đầu ca") && this.dataAddExcel[0][i]["Đầu ca"] != "") {
                a.FirstHaftLeave = true;
            } else {
                a.FirstHaftLeave = false;
            }
            if (this.dataAddExcel[0][i].hasOwnProperty("Cuối ca") && this.dataAddExcel[0][i]["Cuối ca"] != "") {
                a.LastHaftLeave = true;
            } else {
                a.LastHaftLeave = false;
            }
        ...
        taLeaveRegistrationApi.AddLeaveRegistrationFromExcel(arrData).then((res) => {
            ...
            if (!isNullOrUndefined(res.status) && res.status === 200 && res.data == "") {
                ...
                this.importErrorMessage = this.$t('ImportLeaveRegisterErrorMessage') + res.data.toString() + " " + this.$t('Row');
    ...
    processFile(e) {
        if ((<HTMLInputElement>e.target).files.length > 0) {
            ...
            if (!isNullOrUndefined(file)) {
                ...
                (document.getElementById("fileUploadLeave") as any).value = null;
            }
        }
    }
