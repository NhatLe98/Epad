...
export default class RegisterEmployeeShift extends Mixins(TabBase) {
	...
	dataDepartmentFilterChange(){
		this.filterModel.SelectedDepartmentFilter = this.departmentFilter;
	}

	@Watch("employeeFilter")
	dataEmployeeFilterChange(){
		this.filterModel.SelectedEmployeeATIDFilter = this.employeeFilter;
    ...
    onChangeClick(){
        ...
        console.log("dataEmployeeFilterChange ~ this.filterModel:")
		this.loadData();
	}
    LoadDepartmentTree() {
    ...
    async loadData() {
        ...
        const request: EmployeeShiftRequest = {
            ...
            EmployeeATIDs: this.filterModel.SelectedEmployeeATIDFilter,
            DepartmentIDs: this.filterModel.SelectedDepartmentFilter,
            page: this.page,
    ...
    onChangeDepartmentFilter(departments) {
		this.filterModel.SelectedEmployeeATIDFilter = [];
		if (departments && departments.length > 0) {
            const filterEmployees = Misc.cloneData(this.listAllEmployee.filter(x => departments.includes(x.DepartmentIndex)));
            if(filterEmployees && filterEmployees.length > 0){
                this.employeeFullLookupForm = filterEmployees.map(x => ({
