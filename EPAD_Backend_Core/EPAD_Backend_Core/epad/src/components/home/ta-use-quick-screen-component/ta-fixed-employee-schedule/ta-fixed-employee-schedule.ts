...
import { ScheduleFixedEmployeeRequest, TA_ScheduleFixedEmployeeDTO, taScheduleFixedByEmployee } from '@/$api/ta-schedule-fixed-employee-api';
import { TA_ScheduleFixedDepartmentDTO } from '@/$api/ta-schedule-fixed-department-api';
import TableToolbar from '@/components/home/printer-control/table-toolbar.vue';
import * as XLSX from 'xlsx';
@Component({
	name: 'ta-fixed-employee-schedule',
	components: {
...
export default class FixedEmployeeSchedule extends Mixins(TabBase) {
	page = 1;
	showDialog = false;
	showMessage = false;
	checked = false;
	...
	EmployeeATIDs = [];
	isEdit = false;
	formModel: TA_ScheduleFixedEmployeeDTO = {
		DepartmentList: [],
		EmployeeATIDs: [],
		FromDate: new Date,
		ToDate: null,
	...
	columnDefs = [
		...
		{
			field: 'EmployeeATID',
			headerName: this.$t('EmployeeATID'),
			pinned: true,
			width: 120,
			sortable: true,
			display: true
	...
	filterModel = { SelectedDepartmentFilter: [], SelectedEmployeeATIDFilter: [] };
	...
	async beforeMount() {
		...
		if(this.masterEmployeeFilter && this.masterEmployeeFilter.length > 0){
			this.selectedEmployee = Misc.cloneData(this.masterEmployeeFilter)
...
leCommand(command) {
	...
	exportInfoShift() {
		taScheduleFixedByEmployee.ExportInfoShift().then(res => {
	   });
   }
	...
	view(){
		...
		this.filterModel.SelectedEmployeeATIDFilter = this.selectedEmployee;
		this.$emit('filterModel', this.filterModel);
		this.page = 1;
		(this.$refs.scheduleEmployee as any).lPage = this.page;
		this.loadData();
	}
	...
	async loadData() {
		...
		const filter: ScheduleFixedEmployeeRequest = {
			departmentIndexes: this.selectedDepartment,
			employeeATIDs: this.selectedEmployee,
			fromDate: this.fromDateFilter,
			page: this.page,
			pageSize: this.pageSize
		}
		return taScheduleFixedByEmployee.GetScheduleFixedByEmployeeAtPage(filter).then((res) => {
    ...
    selectAllEmployeeForm(value) {
		 (this.formModel as any).EmployeeATIDs = [...value];
        this.$forceUpdate();
        (this.$refs.fixedScheduleEmployee as any).validate();
	}

	reset() {
		const obj: TA_ScheduleFixedEmployeeDTO = {
			DepartmentList: [],
			EmployeeATIDs: [],
			FromDate: new Date,
			ToDate: null,
	...
	onInsertClick() {
		...
		this.reset();
		if(this.masterEmployeeFilter && this.masterEmployeeFilter.length > 0){
			this.formModel.EmployeeATIDs = Misc.cloneData(this.masterEmployeeFilter)
	...
	async onSubmitClick() {
		...
		(this.$refs.fixedScheduleEmployee as any).validate(async (valid) => {
			if (!valid) return;		
			if (this.isEdit == true) {
					return await taScheduleFixedByEmployee.UpdateScheduleFixedByEmployee(submitData).then((res) => {
						if(res.data != true){
							this.$alert(this.$t("FixedEmployeeScheduleExist").toString() + ": " + `${res.data}` + ". " + this.$t("PleaseCheckAgain").toString(), this.$t("Notify").toString(), { dangerouslyUseHTMLString: true }
							);
						}else{
							this.loadData();
							this.showDialog = false;
					...
					return await taScheduleFixedByEmployee
						.AddScheduleFixedByEmployee(submitData)
						.then((res) => {
							if(res.data != true){
								this.$alert(this.$t("FixedEmployeeScheduleExist").toString() + ": " + `${res.data}` + ". " + this.$t("PleaseCheckAgain").toString(), this.$t("Notify").toString(), { dangerouslyUseHTMLString: true }
								);
	...
	async Delete() {
		...
		if (listIndex.length < 1) {
			...
			await taScheduleFixedByEmployee
					.DeleteScheduleFixedByEmployee(listIndex)
					.then((res) => {
						if (!isNullOrUndefined(res.status) && res.status === 200) {
							this.$deleteSuccess();
							this.loadData();
    ...
    onCancelClick() {
        (this.$refs.fixedScheduleEmployee as any).resetFields();
        (this.$refs.fixedScheduleEmployee as any).clearValidate();
        this.showDialog = false;
        this.selectedRows = [];
		this.reset();
    ...
    UploadDataFromExcel() {
        ...
        taScheduleFixedByEmployee.AddScheduleFixedByEmployeeFromExcel(arrData).then((res) => {
            ...
            if (!isNullOrUndefined(res.status) && res.status === 200 && res.data == "") {
                ...
                this.importErrorMessage = this.$t('ImportScheduleEmployeeErrorMessage') + res.data.toString() + " " + this.$t('Row');
    ...
    processFile(e) {
        if ((<HTMLInputElement>e.target).files.length > 0) {
            ...
            if (!isNullOrUndefined(file)) {
				...
				(document.getElementById("fileUploadScheduleEmployee") as any).value = null;
            }
        }
    }
