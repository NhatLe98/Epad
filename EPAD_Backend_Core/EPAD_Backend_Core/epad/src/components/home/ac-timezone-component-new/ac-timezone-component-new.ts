...
export default class TimezoneNewComponent extends Mixins(ComponentBase) {
	page = 1;
	showDialog = false;
	showMessage = false;
	checked = false;
	...
	handleCurrentController(val) {
		...
		this.ruleForm = val;
		this.isEdit = true;
		if(this.ruleForm.UIDIndex){
			this.ruleForm.UIDIndex2 = Number.parseInt(this.ruleForm.UIDIndex) + 1;
			this.ruleForm.UIDIndex3 = Number.parseInt(this.ruleForm.UIDIndex) + 2;
		}
	}
	...
	async getAllData() {
		await timezoneApi.GetAllTimezoneAtPage().then((res) => {
			console.log(res);
			this.tblTimezone = res.data;
		});
	}
