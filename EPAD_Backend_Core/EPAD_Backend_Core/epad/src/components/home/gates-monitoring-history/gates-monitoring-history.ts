...
@Component({
	name: 'gates-monitoring-history-page',
	components: { DataTableComponent, SelectTreeComponent, SelectDepartmentTreeComponent, HeaderComponent }
})
export default class GatesMonitoringHistoryPage extends Mixins(ComponentBase) {
	...
	EmployeeATIDs = [];
	rulesWarning = [];
	ruleObject: GatesMonitoringHistoryModel = {
	...
	createColumnHeader() {
		this.columns = [
			...
			// {
			// 	name: 'Image', dataField: 'Image',
			// 	Fixed: false, width: 150, search: false,    dataType: 'image',  
			// },
			{
				label: 'Note', 
				prop: 'Note',
	...
	async getData({ page, filter, sortParams, pageSize }) {
		...
		return await gatesMonitoringHistoryApi.GetGatesMonitoringHistories(submitData)
			.then((res: any) => {
	...
	async getListDepartment() {
		await departmentApi.GetDepartmentTreeEmployeeScreen("10").then((res: any) => {
			if (res.status == 200) {
	...
	async exportClick() {
		...
		await gatesMonitoringHistoryApi.ExportHExportGatesHistoryistory(this.ruleObject)
			.then((res: any) => {
				const fileURL = window.URL.createObjectURL(new Blob([res.data]));
				const fileLink = document.createElement("a");

				fileLink.href = fileURL;
				fileLink.setAttribute("download", `GatesMonotoringHistory${moment(this.ruleObject.FromTime).format('YYYY-MM-DD')}_${moment(this.ruleObject.ToTime).format('YYYY-MM-DD')}.xlsx`);
