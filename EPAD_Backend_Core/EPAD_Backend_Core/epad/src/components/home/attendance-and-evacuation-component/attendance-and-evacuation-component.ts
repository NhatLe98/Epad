...
export default class AttendanceAndEvacuation extends Vue {
    ...
    async beforeMount() {
        ...
        attendanceLogApi.GetEmergencyAndEvacuation().then((res: any) => {
            // // console.log("RemainInLogs", res)
            if(res && res.data){
                this.remainInLogData = res.data;
                this.tblGroupDeviceLogData = res.data.EmployeeEmergencyList;
                this.tblGroupDeviceLogDatas = res.data.OtherEmergencyList;
    ...
    setInterval: ReturnType<typeof setInterval> = setInterval(() => {
		if (this.$route.path == "/attendance-and-evacuation") {
