    ...
    <el-container>
      <el-header>
        <el-row>
          <el-col :span="12" class="left">
            <span id="FormName">{{ $t("TotalEmployeePresent") }}</span>
          </el-col>
          <el-col :span="12">
            <HeaderComponent />
          </el-col>
      ...
      <div :class="{ 'full-screen fullscreen-container': isFullScreen() }" style="overflow-y: hidden;">
        <el-main style="margin-top: 0; padding: 5px;" :class="{ 'el-main__full-screen': isFullScreen() }">
          <div class="main total-employee-present" 
          :style="isFullscreenOn ? 'height: calc(100vh);' : 'height: calc(100vh - 40px);'">
            <el-tabs v-model="activeTab" @tab-click="handleClick" style="height: calc(100% - 20px);">
                  ...
                  <el-row style="height: 25%">
                    <el-col :span="11">
                      <div style="text-align: center;">
                        <span style="font-weight: bold; font-size: 42px;">{{ $t('TotalWorkingEmployee') }}:</span><br />
                        <span style="font-weight: bold; font-size: 42px;">{{
                          totalEmployee
                        }}</span>
                      </div>
                    </el-col>
                    ...
                    <el-col :span="11" style="height: 100%">
                      <div style="text-align: center;">
                        <span style="font-weight: bold; font-size: 42px;">{{ $t('LogTime') }}:</span><br />
                        <span style="font-weight: bold; font-size: 42px;">{{ strNow }}</span>
                      </div>
                    </el-col>
                  </el-row>
                  <el-row style="height:75%">
                    <el-col class="colBorder" :span="11">
                      <el-table height="calc(100%)" class="total-employee-present__table total-employee-present__area-table"
                        style="margin-top: 0; text-align: center;"
                        :data="tblGroupDeviceLogData.filter((_, idx) => idx % 2 === 0)" @row-click="rowClicked" border
                        empty-text=" ">
                        <el-table-column prop="GroupDeviceName" :label="$t('Areas')"></el-table-column>
                        <el-table-column :label="$t('Working')">
                          <template #default="scope">
                            {{ scope.row.Employees.length }}
                          </template>
                        </el-table-column>
                        <el-table-column :label="$t('CountEmployee')">
              ...
              <el-tab-pane :label="$t('Department')" name="department" style="height: 100%;">
                <div style="height: 10%;">
                  <el-button @click="toggleFullScreen" style="float: right;">
                    <img v-show="isFullscreenOn" src="@/assets/icons/function-bar/fullscreen_off.svg"
                      style="width: 18px; height: 18px" />
                ...
                <div style="height: 90%;">
                  ...
                  <el-row style="height: 5%">
                    <el-checkbox v-model="isShowOnlyRoot">{{
                      $t("OnlyShowRootDepartment")
                    }}</el-checkbox>
                  </el-row>
                  <el-row style="height: 70%">
                    <el-col class="colBorder" :span="11">
                      <el-table height="calc(100%)" class="total-employee-present__table"
                        style="margin-top: 0; text-align: center;"
                        :data="tblDepartmentLogData.filter((_, idx) => idx % 2 === 0)" @row-click="rowClicked" border
                        empty-text=" ">
                        <el-table-column prop="Department" :label="$t('DepartmentOrSection')"></el-table-column>
                        <el-table-column :label="$t('Working')">
                          <template #default="scope">
                            {{ scope.row.Employees.length }}
                          </template>
          ...
          <el-dialog :title="$t('EmployeeInfo')" :visible.sync="dialogEmployeeInfo">
            <el-row :gutter="10">
              ...
              <el-col :span="24" style="margin-top: 10px" v-if="dialogLoading == false" class="table-dialog">
                <el-table height="300" class="total-employee-present__table" :data="tblDataDetail" border empty-text=" "
                  ref="dataTableRef">
...
<script lang="ts" src="./total-employee-present.ts"></script>
...
.total-employee-present .el-tabs__header{
  margin: 0 !important;
  height: unset;
}

.total-employee-present__table td div,
.total-employee-present__table th div {
  padding-left: 10px !important;
  // text-align: inherit !important;
}

.total-employee-present__table th {
  color: rgb(19, 56, 152);
...
.total-employee-present__area-table tbody tr {
  height: calc(100px);
  td {
    height: 100%;
    div {
...
.total-employee-present__area-table thead tr {
  height: calc(60px);
  th {
    height: 100%;
    div {
...
.total-employee-present__area-table td div,
.total-employee-present__area-table th div {
  padding-left: 10px !important;
  // text-align: inherit !important;
}

.total-employee-present__area-table th {
  color: rgb(19, 56, 152);
