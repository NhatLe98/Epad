...
import { DepartmentAccessedGroupModel, departmentAccessedGroupApi } from "@/$api/gc-department-accessed-group-api";
@Component({
...
export default class DepartmentAccessedGroup extends Mixins(ComponentBase) {
    columns = [];
    rowsObj = [];
    isEdit = false;
    showDialog = false;
    rules = null;
    accessedGroupModel: DepartmentAccessedGroupModel = {
        Index: 0,
        DepartmentIDs: [],
        FromDate: new Date,
        ToDate: null
    ...
    updateFunctionBarCSS() {
        ...
        const component2 = document.querySelector('.department-accessed-group__data-table');
        ...
        const component5 = document.getElementsByClassName('datatable-function department-accessed-group__data-table-function');
    ...
    Edit() {
        ...
        // (this.$refs.departmentAccessedGroupTable as any).getTableData(this.page);
    }
    ...
    Delete() {
        ...
        if (listIndex.length < 1) {
            ...
            this.$confirmDelete().then(() => {
                departmentAccessedGroupApi.DeleteDepartmentAccessedGroup(listIndex).then((res: any) => {
    ...
    async ConfirmClick() {
        (this.$refs.accessedGroupModel as any).validate(async (valid) => {
            ...
            submitData.DepartmentIndex = this.accessedGroupModel.DepartmentIDs[0] || null;
            console.log("DepartmentIndex:", submitData.DepartmentIndex )
            if (this.isEdit == false) {
                await departmentAccessedGroupApi.AddDepartmentAccessedGroup(submitData).then((res: any) => {
                ...
                await departmentAccessedGroupApi.UpdateDepartmentAccessedGroup(submitData).then((res: any) => {
    ...
    async getData({ page, filter, sortParams, pageSize }) {
        this.page = page;
        var request = {
            DepartmentFilter: this.filterDepartment,
        ...
        return await departmentAccessedGroupApi.GetDepartmentAccessedGroupByFilter(request).then((res) => {
    ...
    onChangeDepartmentForm(departments) {
        (this.$refs.accessedGroupModel as any).clearValidate('DepartmentIDs');
    ...
    async ProcessAutoSelectFromExcel() {
        ...
        (this.$refs.departmentAccessedGroupTable as any).getTableData(1).then(() => {
            this.filterModel.ListEmployeeATID = [];
            this.showDialogAutoSelectExcel = false;
        });
    }
    ...
    async UploadDataFromExcel() {
        ...
        await departmentAccessedGroupApi.ImportDepartmentAccessedGroup(arrData).then((res: any) => {
            console.log("awaitdepartmentAccessedGroupApi.ImportDepartmentAccessedGroup ~ res:", res)
            if (!isNullOrUndefined(res.status) && res.status === 200 && res.data == "") {
                this.$saveSuccess();
                    (this.$refs.departmentAccessedGroupTable as any).getTableData(this.page);
            }else{  
                this.importErrorMessage = this.$t('ImportDepartmentAccessedGroupFailed')+ res.data.toString() + " " + this.$t('Row');
