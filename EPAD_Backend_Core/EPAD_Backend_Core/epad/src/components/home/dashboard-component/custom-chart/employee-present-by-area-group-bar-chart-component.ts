...
@Component({
  name: "EmployeePresentByAreaGroupBarChart",
  components: { BarChart, CustomLegendBarChart },
})
export default class EmployeePresentByAreaGroupBarChartComponent extends Mixins(ComponentBase) {
  @Prop() index: any;
  id = "employee-present-by-area-group-bar-chart";
  @Prop() name;
  logData: any;
  chartLabels: any;
  chartSets: any;
  ...
  beforeMount() {
    this.explodeColumns = [
      ...
      {
        prop: 'AreaGroupName',
        label: this.$t('AreaGroupNames'),
      }
    ];

    areaGroupApi.GetAreaGroupAll().then((res: any) => {
      // console.log(res)
      if(res && res.data && res.data.data){
        this.areaGroupData = res.data.data;
        // console.log(this.areaGroupData)
      }
      this.areaGroupData.push({
        Index: 0,
        Name: this.$t('NoAreaGroup'),
      })
    });
    ...
    attendanceLogApi.GetTupleFullWorkingEmployeeByDepartment().then((res: any) => {
      ...
      if (res && res.data) {
        ...
        if (res.data) {
          ...
          if (this.areaGroupData && this.areaGroupData.length > 0) {
            this.areaGroupData.forEach((element, index) => {
              let groupLogInData = [];
              let groupLogOutData = [];
              let groupLogRemainData = [];
              if(element.Index != 0){
                groupLogInData = Misc.cloneData(this.logData.Item1.filter(x => 
                  x.AreaGroupIndex && x.AreaGroupIndex.includes(element.Index)));
                ...
                groupLogInData = Misc.cloneData(this.logData.Item1.filter(x => 
                  !x.AreaGroupIndex || (x.AreaGroupIndex && x.AreaGroupIndex.length == 0)));
                groupLogOutData = Misc.cloneData(this.logData.Item2.filter(x => 
                  !x.AreaGroupIndex || (x.AreaGroupIndex && x.AreaGroupIndex.length == 0)));
                groupLogRemainData = Misc.cloneData(this.logData.Item3.filter(x => 
                  !x.AreaGroupIndex || (x.AreaGroupIndex && x.AreaGroupIndex.length == 0)));    
    ...
    if (chartClickedData) {
      ...
      if(chartClickedData.dataId != 0){
        if(chartClickedData.dataSetId == "In"){
          this.explodeData = departmentLogData.Item1.filter(x => x.AreaGroupIndex 
            && x.AreaGroupIndex.includes(chartClickedData.dataId));
        }else if(chartClickedData.dataSetId == "Out"){
          this.explodeData = departmentLogData.Item2.filter(x => x.AreaGroupIndex 
            && x.AreaGroupIndex.includes(chartClickedData.dataId));
        }else if(chartClickedData.dataSetId == "Remain"){
          this.explodeData = departmentLogData.Item3.filter(x => x.AreaGroupIndex 
            && x.AreaGroupIndex.includes(chartClickedData.dataId));
        }
      }else{
        if(chartClickedData.dataSetId == "In"){
          this.explodeData = departmentLogData.Item1.filter(x => !x.AreaGroupIndex 
            || (x.AreaGroupIndex && x.AreaGroupIndex.length == 0));
        }else if(chartClickedData.dataSetId == "Out"){
          this.explodeData = departmentLogData.Item2.filter(x => !x.AreaGroupIndex 
            || (x.AreaGroupIndex && x.AreaGroupIndex.length == 0));
        }else if(chartClickedData.dataSetId == "Remain"){
      ...
      if (this.explodeData && this.explodeData.length > 0) {
        this.explodeData.forEach((element, index) => {
          if(element.AreaGroupName && element.AreaGroupName.length > 0){
            element.AreaGroupName = element.AreaGroupName.join(', ');
          }else{
            element.AreaGroupName = this.$t('NoAreaGroup');
          }
