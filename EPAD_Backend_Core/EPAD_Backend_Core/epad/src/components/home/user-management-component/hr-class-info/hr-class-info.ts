...
import { hrClassInfoApi, HR_ClassInfo } from '@/$api/hr-class-info-api';
...
export default class HRClassInfoComponent extends Mixins(ComponentBase) {
    page = 1;
    pageSize = 40;
    showDialog = false;
    checked = false;
    ...
    initRule() {
        this.rules = {
            Name: [
                {
                    required: true,
                    message: this.$t('PleaseInputClassName'),
                    trigger: 'blur',
                },
            ],
            Code: [
                {
                    required: true,
                    message: this.$t('PleaseInputClassCode'),
                    trigger: 'blur',
                },
            ]
        };
    ...
    async getData({ page, filter, sortParams, pageSize }) {
        ...
        return await hrClassInfoApi.GetHRClassInfoAtPage(page, filter, pageSize).then((res) => {
            const { data } = res as any;
            this.dataClassInfo = data.data;
            return {
                data: this.dataClassInfo,
                total: data.total,
            };
        });
    }
    ...
    async Submit() {
        (this.$refs.ruleForm as any).validate(async (valid) => {
            if (!valid) return;
                ...
                this.isLoading = true;
                if (this.isEdit == true) {
                    await hrClassInfoApi.UpdateHRClassInfo(classIndex,this.ruleForm).then((res) => {
                    ...
                    await hrClassInfoApi.AddHRClassInfo(this.ruleForm).then((res) => {
                        (this.$refs.table as any).getTableData(this.page, null, null);
                        this.showDialog = false;
                        this.isLoading = false;
                        this.Reset();
    ...
    async Delete() {
        ...
        if (obj.length < 1) {
            ...
            this.$confirmDelete().then(async () => {
                await hrClassInfoApi.DeleteHRClassMulti(obj)
                    .then((res) => {
                        (this.$refs.table as any).getTableData(this.page, null, null);
                        if (!isNullOrUndefined(res.status) && res.status === 200) {
                            this.$deleteSuccess();
