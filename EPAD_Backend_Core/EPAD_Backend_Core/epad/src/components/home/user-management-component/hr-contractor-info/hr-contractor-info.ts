...
import { hrContractorInfoApi } from '@/$api/hr-contractor-info-api';

@Component({
    name: 'hr-contractor-info',
    components: { 't-grid': TGrid },
})
export default class HRContractorInfo extends Mixins(TabBase) {
    ...
    async loadData() {
        this.dataSource = [];
        await hrContractorInfoApi.GetContractorAtPage( this.page, this.pageSize).then(response => {
