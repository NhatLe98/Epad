...
import { hrTeacherInfoApi } from '@/$api/hr-teacher-info-api';

@Component({
    name: 'hr-teacher-info',
    components: { 't-grid': TGrid },
})
export default class HRTeacherInfo extends Mixins(TabBase) {
    ...
    async loadData() {
        this.dataSource = [];
        await hrTeacherInfoApi.GetTeacherAtPage( this.page, this.pageSize).then(response => {
